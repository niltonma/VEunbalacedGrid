...
ls_f2_v2 = functions.create_custom_ls_comb('C:\\Users\\alves\\AppData\\Local\\OpenDSS\\IEEE13Nodeckt_Mon_powers1_comb_1.csv','C:\\Users\\alves\\Downloads\\IEEE13Nodeckt_Mon_powers1_comb.csv',ls_comb)


dss.text(f"New LoadShape.comb npts={n_pontos_curva}  interval={0.25}  mult={ls_comb}")
dss.text(f"New LoadShape.comb_v2 npts={n_pontos_curva}  interval={0.25}  mult={ls_f2_v2}")
#case C (G2V): adicionar carga à carga original
#valores à adicionar:
dss.text("New Load.634a1 Bus1=634.1     Phases=1 Conn=Wye  Model=1 kV=0.277  kW=252   kvar=0 daily=comb")
dss.text("New Load.634b1 Bus1=634.2     Phases=1 Conn=Wye  Model=1 kV=0.277  kW=168   kvar=0 daily=comb_v2")
dss.text("New Load.634c1 Bus1=634.3     Phases=1 Conn=Wye  Model=1 kV=0.277  kW=168   kvar=0 daily=comb_v2") 
