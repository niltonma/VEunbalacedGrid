...
dss = py_dss_interface.DSS.DSSDLL()
...
for i in range(24):
    ...
    dss.text("Set number = 1")
    dss.text("solve")
    for node in range(len(dss.circuit_all_node_names()[3:])):
        if ".4" in dss.circuit_all_node_names()[3:][node]:
            continue
        else:
            nodes_vpu_list.append(dss.circuit_all_bus_vmag_pu()[3:][node])
