...
import os, functions, funcoes, random

mode =1 #0: caso base, 1: com carregamento
num_H = 24
...
if mode == 0: ##  caso base
    ...
    Tensao_Nos = pd.DataFrame(v_mag_pu, columns= node_list)
    print(Tensao_Nos)
    # plt.figure(); 
    # Tensao_Nos.plot();
    # plt.legend(loc='lower left')
    tensao_fase1 = Tensao_Nos.iloc[:,Tensao_Nos.columns.str.contains(r'(.1$)')]
    tensao_fase2 = Tensao_Nos.iloc[:,Tensao_Nos.columns.str.contains(r'(.2$)')]
    tensao_fase3 = Tensao_Nos.iloc[:,Tensao_Nos.columns.str.contains(r'(.3$)')]
    
    plt.rcParams["figure.figsize"] = [7.50, 3.50]
    plt.rcParams["figure.autolayout"] = True
    fig, ax = plt.subplots()
    tensao_fase1.plot(ax=ax,  legend=True)
    ...
    plt.rcParams["figure.autolayout"] = True
    fig, ax = plt.subplots()
    tensao_fase2.plot(ax=ax,  legend=True)
    ...
    plt.rcParams["figure.autolayout"] = True
    fig, ax = plt.subplots()
    tensao_fase3.plot(ax=ax,  legend=True)
    ...
    for bus in selected_buses:
        ...
        dss.text(f"New Load.{bus}abc1 Bus1={bus}.1   Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-252   kvar=0 daily=Semana")
        dss.text(f"New Load.{bus}abc2 Bus1={bus}.2   Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-168 daily=Semana")
        dss.text(f"New Load.{bus}abc3 Bus1={bus}.3   Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-168 daily=Semana")
    ...
    plt.rcParams["figure.autolayout"] = True
    fig, ax = plt.subplots()
    tensao_fase1.loc[:,['675.1', '692.1']].plot(ax=ax,  legend=True)
    plt.title('Tensao Fase 1')
    plt.xlabel('Horas')
    plt.ylabel('V [pu]')
    plt.show()
