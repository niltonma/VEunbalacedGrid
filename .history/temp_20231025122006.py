...
dss.text("New monitor.powers1 action=Save element=Transformer.XFM1 \\
          terminal=2 ppolar=no mode=1") # mode= 1 medir Potencia ativa
#carga original, caso base - CASE A:
dss.text("edit Load.634a Bus1=634.1  \\
            Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-160   kvar=-110 daily=DEFAULT")
dss.text("edit Load.634b Bus1=634.2   \\
           Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-120   kvar=-90  daily=DEFAULT")
dss.text("edit Load.634c Bus1=634.3   \\
          Phases=1 Conn=Wye  Model=1 kV=0.277  kW=-120   kvar=-90  daily=DEFAULT")
dss.solution_solve()
