...
v_mag_EOL_pu = pd.DataFrame(index=nodes_names, columns=range(n_pontos_curva))
v_mag_EOL = pd.DataFrame(index=nodes_names, columns=range(n_pontos_curva))
...
for h in range(n_pontos_curva):
    ...
    dss.text("solve")
    # v_mag_EOL[h,:] = dss.circuit.buses_vmag
    v_mag_EOL[h]= dss.circuit.buses_vmag
    v_mag_EOL_pu[h] = dss.circuit.buses_vmag_pu
plt.plot(v_mag_EOL_pu.loc["rg60.1",:])
#############################3
